...
const SSHKeyInput = (props: ControlProps & MutableInput) => {
  // Do not use handleChange, this is the version that is not debounced.
  const { id, enabled, data, schema, errors, visible, onChange } = props;
  ...
  const freezeInput = (keyValue: string) => {
    setKey(keyValue);
    onChange(keyValue);
  ...
  let placeholder = 'Click to select file...';
  if ((schema as any).example != undefined) {
    placeholder = (schema as any).example;
  }
