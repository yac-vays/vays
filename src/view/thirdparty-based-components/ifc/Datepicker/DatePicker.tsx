...
/**
 * Date picker component. Note that the date tme format can be customized according to
 * https://flatpickr.js.org/formatting/.
 * @returns
 */
const DatePicker = (props: DatePickerProps) => {
  const { onChange, title, format, type, enabled, description, required } = props;
  ...
  useEffect(() => {
    ...
    flatpickr('.form-datepicker', {
      ...
      dateFormat: 'Z', // What the server receives
      altInput: true,
      ...
      <div className={`${enabled ? 'opacity-100' : 'opacity-50'}`}>
        <div className="relative">
          <input
            ...
            onInput={(e) =>
              onChange(
                type === 'date' ? e.currentTarget.value.split('T')[0] : e.currentTarget.value,
              )
            }
          />
